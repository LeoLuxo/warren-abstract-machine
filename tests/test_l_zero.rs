mod l_zero {
	...
	fn test_simple() -> Result<()> {
		...
		assert_eq!(
			solve::<L0>("test(f(c))".parse_as()?, "test(C)".parse_as()?)?,
			"{ C -> f(c) }".parse_as()?
		);
	...
	fn test_unbound() -> Result<()> {
		assert_eq!(
			solve::<L0>("p(A, B, A)".parse_as()?, "p(X, Y, Z)".parse_as()?)?,
			"{ X -> ?1, Y -> ?2, Z -> ?1 }".parse_as()?
		);

		assert_eq!(
			solve::<L0>("p(A, B, B)".parse_as()?, "p(X, Y, Z)".parse_as()?)?,
			"{ X -> ?1, Y -> ?2, Z -> ?2 }".parse_as()?
		);

		assert_eq!(
			solve::<L0>("p(A, A, B)".parse_as()?, "p(X, Y, Z)".parse_as()?)?,
