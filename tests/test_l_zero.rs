mod l_zero {
	...
	fn test_simple() -> Result<()> {
		assert_eq!(
			format!("{}", solve::<L0>("p(a)".parse()?, "p(X)".parse()?)?),
			"{ X -> a }"
		);

		assert_eq!(
			format!("{}", solve::<L0>("test(f(c))".parse()?, "test(C)".parse()?)?),
			"{ C -> f(c) }"
		);
	...
	fn test_wambook() -> Result<()> {
		assert_eq!(
			format!(
				"{}",
				solve::<L0>("p(f(X), h(Y, f(a)), Y)".parse()?, "p(Z, h(Z,W), f(W))".parse()?)?
			),
			"{ W -> f(a), Z -> f(f(a)) }"
		);
	...
	fn test_unbound() -> Result<()> {
		assert_eq!(
			format!("{}", solve::<L0>("p(A, B, A)".parse()?, "p(X, Y, Z)".parse()?)?),
			"{ X -> ?1, Y -> ?2, Z -> ?1 }"
		);

		assert_eq!(
			format!("{}", solve::<L0>("p(A, B, B)".parse()?, "p(X, Y, Z)".parse()?)?),
			"{ X -> ?1, Y -> ?2, Z -> ?2 }"
		);

		assert_eq!(
			format!("{}", solve::<L0>("p(A, A, B)".parse()?, "p(X, Y, Z)".parse()?)?),
			"{ X -> ?1, Y -> ?1, Z -> ?2 }"
		);
