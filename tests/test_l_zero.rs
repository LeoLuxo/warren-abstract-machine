mod l_zero {
	use anyhow::Result;
	use warren_abstract_machine::{l_zero::L0, parser::ParseAs, solve};

	#[test]
	fn test_simple() -> Result<()> {
		assert_eq!(
			format!("{}", solve::<L0>("p(a)".parse_as()?, "p(X)".parse_as()?)?),
			"{ X -> a }"
		);

		assert_eq!(
			format!("{}", solve::<L0>("test(f(c))".parse_as()?, "test(C)".parse_as()?)?),
			"{ C -> f(c) }"
		);
	...
	fn test_broad() -> Result<()> {
		...
		format!(
			"{}",
			solve::<L0>("f(AA,AB,AC,AD,AE,AR,AF,AG,AH,AI,AJ,AK,AL,AM,AN,AO,AP,AQ,AR,AS,AT,AU,AV,AW,AX,AY,AZ,BA,BB,BC,BD,BE,BR,BF,BG,BH,BI,BJ,BK,BL,BM,BN,BO,BP,BQ,BR,BS,BT,BU,BV,BW,BX,BY,BZ,CA,CB,CC,CD,CE,CR,CF,CG,CH,CI,CJ,CK,CL,CM,CN,CO,CP,CQ,CR,CS,CT,CU,CV,CW,CX,CY,CZ)".parse_as()?, "f(AA,AB,AC,AD,AE,AR,AF,AG,AH,AI,AJ,AK,AL,AM,AN,AO,AP,AQ,AR,AS,AT,AU,AV,AW,AX,AY,AZ,BA,BB,BC,BD,BE,BR,BF,BG,BH,BI,BJ,BK,BL,BM,BN,BO,BP,BQ,BR,BS,BT,BU,BV,BW,BX,BY,BZ,CA,CB,CC,CD,CE,CR,CF,CG,CH,CI,CJ,CK,CL,CM,CN,CO,CP,CQ,CR,CS,CT,CU,CV,CW,CX,CY,CZ)".parse_as()?)?
		),
	...
	fn test_unbound() -> Result<()> {
		assert_eq!(
			format!("{}", solve::<L0>("p(A, B, A)".parse_as()?, "p(X, Y, Z)".parse_as()?)?),
			"{ X -> ?1, Y -> ?2, Z -> ?1 }"
		);

		assert_eq!(
			format!("{}", solve::<L0>("p(A, B, B)".parse_as()?, "p(X, Y, Z)".parse_as()?)?),
			"{ X -> ?1, Y -> ?2, Z -> ?2 }"
		);

		assert_eq!(
			format!("{}", solve::<L0>("p(A, A, B)".parse_as()?, "p(X, Y, Z)".parse_as()?)?),
			"{ X -> ?1, Y -> ?1, Z -> ?2 }"
		);
